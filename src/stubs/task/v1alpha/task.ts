...
class ListTasksResponse$Type extends MessageType<ListTasksResponse> {
    constructor() {
        super("task.v1alpha.ListTasksResponse", [
            { no: 1, name: "task", kind: "message", repeat: 1 /*RepeatType.PACKED*/, T: () => Task },
        ...
        while (reader.pos < end) {
            ...
            switch (fieldNo) {
                case /* repeated task.v1alpha.Task task */ 1:
                    message.task.push(Task.internalBinaryRead(reader, reader.uint32(), options));
                    break;
    ...
    internalBinaryWrite(message: ListTasksResponse, writer: IBinaryWriter, options: BinaryWriteOptions): IBinaryWriter {
        ...
        for (let i = 0; i < message.task.length; i++)
            Task.internalBinaryWrite(message.task[i], writer.tag(1, WireType.LengthDelimited).fork(), options).join();
